...
export const fetchPointsStatistics = async () => {
  try {
    ...
    const pendingUnlocked = pendingLleUnlockedKeys.map((key) => Number(data[key])).reduce((a, b) => a + b, 0);
    const locked = lleLockedKeys.map((key) => Number(data[key])).reduce((a, b) => a + b, 0);
    const pendingLocked = pendingLleLockedKeys.map((key) => Number(data[key])).reduce((a, b) => a + b, 0);
